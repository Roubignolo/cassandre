...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if(empty($nom_err) && empty($actif_err) && empty($image_err)){
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            mysqli_stmt_bind_param($stmt, "sss", $param_nom, $param_actif, $param_image, $param_id);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        <div class="form-group <?php echo (!empty($nom_err)) ? 'has-error' : ''; ?>">
                            <label>Nom</label>
                            <input type="text" nom="nom" class="form-control" value="<?php echo $nom; ?>">
                            <span class="help-block"><?php echo $nom_err;?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($actif_err)) ? 'has-error' : ''; ?>">
                            <label>Actif</label>
                            <textarea nom="actif" class="form-control"><?php echo $actif; ?></textarea>
                            <span class="help-block"><?php echo $actif_err;?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($image_err)) ? 'has-error' : ''; ?>">
                            <label>Logo</label>
                            <input type="text" nom="image" class="form-control" value="<?php echo $image; ?>">
                            <span class="help-block"><?php echo $image_err;?></span>
                        </div>
                        <input type="hidden" nom="id" value="<?php echo $id; ?>"/>
